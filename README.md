...
1. **下载并运行安装包**：
   双击运行 `build\installer\ETS2-Chat-Translator-Manager-Setup-0.3.18.exe` 进行安装。
...
└── installer/
    └── ETS2-Chat-Translator-Manager-Setup-0.3.18.exe # 独立安装包 (集成 NSIS)
```
...
### 🚀 v0.3.19
* **🎯 适配欧卡 1.61**：补齐 SCS Telemetry SDK 版本定义（1.00~1.14），版本协商改为按主版本匹配，游戏更新后插件仍可正常加载。
* **🔌 接入 TruckersMP 官方 Client SDK**：同一个 DLL 同时实现 SCS Telemetry 与 TruckersMP SDK 接口，可直接读取附近玩家的身份信息。
* **🧑‍✈️ 管理员身份标注**：聊天中为管理员等身份显示徽章，分级为 `ADMIN`（管理层）、`GM`（游戏管理员）、`TEAM`（团队成员）、`VIP`（赞助者），并兼容 `[TAG]` 前缀与大小写、空格差异。
* **🐛 修复发送消息定位错误**：此前按键会打到悬浮窗自身导致“发送超时”，现在会排除悬浮窗并正确定位游戏主窗口，同时在日志记录实际发送目标窗口。
* **✨ 修复悬浮窗闪烁**：渲染统一转交 UI 线程并合并重绘请求、复用位图缓存，消除跨线程绘制与重复刷新造成的闪烁。
* **⚡ 中译英发送优化**：不再隐藏/重显悬浮窗（消除发送瞬间的闪屏），改为轮询等待游戏窗口激活，剪贴板自动还原。
* **🎨 UI 调整**：连接/服务信息改用低调配色（仅处罚类消息使用警示色），状态栏精简，底部提示 6 秒后自动消失。

### 🚀 v0.3.18
* **🎮 发送中译英内容**：支持中译英发送至游戏，翻译完成后自动发送消息到游戏公屏，并通过聊天日志确认发送结果。
* **⚠️ 测试提示**：中文翻译英文自动发送公屏功能暂未实机充分测试，如有问题请提交 Issues。
* **⚡ 发送速率优化**：减少自动打开聊天框、粘贴、回车发送之间的固定等待，发送后用日志确认兜底。

### 🚀 v0.3.17
* **🎮 悬浮窗中文输入框**：新增悬浮窗中文输入框，可将中文翻译为目标语言并粘贴到游戏聊天框。
* **🧷 粘贴流程优化**：增加忙碌状态、剪贴板恢复和游戏窗口校验，降低误发风险。
...
* **🚫 无意义消息过滤**：`？？？？、`、纯标点、纯数字等内容会直接原样显示，不再提交翻译接口，也不会卡在“翻译中”。
* **🧩 跳过翻译显示优化**：悬浮窗隐藏、翻译器未就绪或无需翻译时，会显示原消息，避免出现误导性的失败提示。
